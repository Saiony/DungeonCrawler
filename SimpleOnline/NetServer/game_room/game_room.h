...
namespace dungeon_server::game_room
{
    ...
    private:
        ...
        domain::encounter get_encounter() const;
        std::function<void(std::shared_ptr<domain::message::emitter_message>)> send_inner_message_;
    public:
        explicit game_room(std::vector<domain::player> players, const std::function<void(std::shared_ptr<domain::message::emitter_message>)>& send_message_function);
