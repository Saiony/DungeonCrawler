...
namespace dungeon_server::domain
{
    ...
    {
    protected:
        virtual void on_execute_turn(const std::shared_ptr<encounter>& encounter_ptr, action_log& action_log) = 0;
    public:
        virtual std::string get_base_name() = 0;
        base_enemy(uint16_t health, uint16_t attack_damage, uint16_t ability_power,
                   dungeon_common::enums::elemental_property_type elemental_property);
        
        void execute_turn(const std::shared_ptr<encounter>& encounter_ptr, action_log& action_log)
        {            
            if(!can_execute_turn())
                return;
