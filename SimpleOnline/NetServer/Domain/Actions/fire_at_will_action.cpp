...
void dungeon_server::domain::action::fire_at_will_action::use(const std::shared_ptr<encounter>& encounter_ptr, action_log& action_log)
{
    const uint8_t shot_count = 3;
    const auto action_owner = encounter_ptr->get_creature(action_owner_id);

    action_log.add_log(action_owner->name + " used " + get_name());
    for (int i = 0; i < shot_count; ++i)
    {
        action_log.add_log("An arrow takes flight...");
