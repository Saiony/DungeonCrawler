...
void dungeon_server::domain::action::frostbite_action::use(const std::shared_ptr<encounter>& encounter_ptr, action_log& action_log)
{
    ...
    if(percentage <= freeze_chance_)
    {
        ...
        action_log.add_log(target->name + " is frozen");
    }
