...
dungeon_server::domain::action::distracting_slash_action::~distracting_slash_action() = default;
...
void dungeon_server::domain::action::distracting_slash_action::use(const std::shared_ptr<encounter>& encounter_ptr, action_log& action_log)
{
    ...
    if (new_attacker_it != std::end(encounter_ptr->enemies))
    {
        ...
        action_log.add_log(" and created an opportunity for " + new_attacker->name);
