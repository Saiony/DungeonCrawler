...
void dungeon_server::domain::stuck_in_spiders_web_status::on_attacked(const std::shared_ptr<encounter>& encounter, std::string& action_log,
...
void dungeon_server::domain::stuck_in_spiders_web_status::end_status(const std::shared_ptr<encounter>& encounter, std::string& action_log)
{
    const auto this_creature = *std::ranges::find_if(encounter->creatures, [this](auto creature)
    {
        return creature->public_id == creature_id_;
    ...
    action_log += "\nThe web rips and " + this_creature->name + " is no longer restrained";
}
